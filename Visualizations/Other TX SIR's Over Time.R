...
county_SIRs <- read_excel("GitHub/cprit_2020/Analysis/Clean Data for Plots/Other County SIRs All Years.xlsx")
...
plot1 <- ggplot(tx_SIR_map_sf) + geom_sf(aes(fill = SIR)) +
  ...
  ggtitle("Other Non-Small Cell Carcinoma SIR's Over Time") + theme_bw() +
  theme(
...
plot3 <- ggplot(tx_SIR_map_sf) + geom_sf(aes(fill = RR)) +
  facet_wrap(~Year, dir = "h", ncol = 3) +
  ggtitle("Other Non-Small Cell Carcinoma SIR's Over Time After INLA Smoothing") + theme_bw() +
  theme(
    axis.text.x = element_blank(),
    axis.text.y = element_blank(),
    axis.ticks = element_blank()
